  ...
  try {
    ...
    const earningsResult = await pool.query(`
      SELECT
        ...
        COALESCE(SUM(CASE WHEN status = 'PAID' THEN amount ELSE 0 END), 0) as total_paid_earnings,
        COALESCE(SUM(CASE WHEN status = 'PENDING' THEN amount ELSE 0 END), 0) as total_pending_earnings
      FROM earnings
    `);
