...
if (!base || !email || !password) {
  throw new Error('Missing BASE_URL or super admin credentials');
}
...
must(adminMe.status === 200, `admin me failed: ${JSON.stringify(adminMe.json)}`);
...
const agentPhone = `070${Date.now().toString().slice(-8)}`;
...
console.log(JSON.stringify({
  ...
  cpA: cpA.json.data.id,
  cpB: cpB.json.data.id,
  picker: picker.json.data.id,
  agent: agentUser.json.data.id,
  logA: logA.json.data.id,
  logB: logB.json.data.id,
